...
function renderServiceLibrary(purchases) {
  const services = SERVICE_CATALOG.map((service, catalogIndex) => ({
    service,
    purchase: findPurchasedService(service, purchases),
    catalogIndex,
  })).sort((left, right) => Number(Boolean(right.purchase)) - Number(Boolean(left.purchase)) || left.catalogIndex - right.catalogIndex);
