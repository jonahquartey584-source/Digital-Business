...
export default async (request: Request) => {
  ...
  const session = await response.json().catch(() => ({})) as { payment_status?: string; status?: string; metadata?: { portal_code?: string; account?: string } };
  ...
  return Response.json({ status: "paid", code: code.replace(/(\d{4})(?=\d)/g, "$1 "), account: session.metadata?.account || "" }, { headers: { "Cache-Control": "no-store" } });
};
